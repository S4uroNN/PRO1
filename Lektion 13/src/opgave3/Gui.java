...
//		pane.add(txfName, 1, 0,2,1);
		...
		pane.add(txfCombine, 1, 0, 2, 1);


		// add a button to the pane (at col=1, row=1)
		Button btnSplit = new Button("Swap");
		pane.add(btnSplit, 1, 2);
		GridPane.setMargin(btnSplit, new Insets(10, 10, 0, 0));
...
//		//---------------------------------------------------------------------------------------
	...
	private void split() {
		String combine = txfCombine.getText().trim();
		int space = 0;
		if(combine.contains(" ")){
			space = combine.indexOf(" ");
			txfFirstName.setText(combine.substring(0,space));
			txfLastName.setText(combine.substring(space,combine.length()));
		}
	}
}
